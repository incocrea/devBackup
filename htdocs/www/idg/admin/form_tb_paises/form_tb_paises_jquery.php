...
function scEventControl_init(iSeqRow) {
  scEventControl_data["pais_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["id_pais_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["pais_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["pais_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["id_pais_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_pais_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_pais_' + iSeqRow).bind('blur', function() { sc_form_tb_paises_id_pais__onblur(this, iSeqRow) })
                                      .bind('change', function() { sc_form_tb_paises_id_pais__onchange(this, iSeqRow) })
                                      .bind('focus', function() { sc_form_tb_paises_id_pais__onfocus(this, iSeqRow) });
  $('#id_sc_field_pais_' + iSeqRow).bind('blur', function() { sc_form_tb_paises_pais__onblur(this, iSeqRow) })
                                   .bind('change', function() { sc_form_tb_paises_pais__onchange(this, iSeqRow) })
                                   .bind('focus', function() { sc_form_tb_paises_pais__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_tb_paises_id_pais__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_paises_validate_id_pais_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_paises_id_pais__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_paises_id_pais__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_tb_paises_pais__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_paises_validate_pais_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_paises_pais__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_paises_pais__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("pais_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_pais_(row, status);
	displayChange_field_id_pais_(row, status);
}
...
function displayChange_field_pais_(row, status) {
    var fieldId;
}

function displayChange_field_id_pais_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_tb_paises_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_tb_paises')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
