...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["colorestado_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["colorestado_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["colorestado_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_estado_' + iSeqRow).bind('change', function() { sc_form_tb_estados_id_estado__onchange(this, iSeqRow) });
  $('#id_sc_field_descripcion_' + iSeqRow).bind('blur', function() { sc_form_tb_estados_descripcion__onblur(this, iSeqRow) })
                                          .bind('change', function() { sc_form_tb_estados_descripcion__onchange(this, iSeqRow) })
                                          .bind('focus', function() { sc_form_tb_estados_descripcion__onfocus(this, iSeqRow) });
  $('#id_sc_field_colorestado_' + iSeqRow).bind('blur', function() { sc_form_tb_estados_colorestado__onblur(this, iSeqRow) })
                                          .bind('change', function() { sc_form_tb_estados_colorestado__onchange(this, iSeqRow) })
                                          .bind('focus', function() { sc_form_tb_estados_colorestado__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_tb_estados_id_estado__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_tb_estados_descripcion__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_estados_validate_descripcion_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_estados_descripcion__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_estados_descripcion__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_tb_estados_colorestado__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_estados_validate_colorestado_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_estados_colorestado__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_estados_colorestado__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	...
	displayChange_field("colorestado_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_descripcion_(row, status);
	displayChange_field_colorestado_(row, status);
}
...
function displayChange_field_colorestado_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_tb_estados_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_tb_estados')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
