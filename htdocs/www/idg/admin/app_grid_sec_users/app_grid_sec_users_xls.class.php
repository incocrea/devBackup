...
function actionBar_isValidState($buttonName, $buttonState)
{
   ...
   { 
      ...
      { 
          ...
          if ($Cada_col == "mfa" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              $SC_Label = NM_charset_to_utf8($SC_Label);
   ...
   function NM_export_mfa()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->mfa = html_entity_decode($this->mfa, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->mfa = strip_tags($this->mfa);
         $this->mfa = NM_charset_to_utf8($this->mfa);
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->mfa, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->mfa, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- picture
   ...
   //----- mfa
   function NM_sub_cons_mfa()
   {
         $this->mfa = html_entity_decode($this->mfa, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->mfa = strip_tags($this->mfa);
         $this->mfa = NM_charset_to_utf8($this->mfa);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->mfa;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
