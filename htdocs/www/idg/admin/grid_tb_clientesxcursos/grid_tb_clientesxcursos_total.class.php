...
class grid_tb_clientesxcursos_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_tb_clientesxcursos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_tb_clientesxcursos']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $cliente_id, $estado_id;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), count(Cliente_id) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['contr_total_geral'] = "OK";
   } 

   //-----  curxinst_id
   function quebra_curxinst_id_sc_free_group_by($curxinst_id, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $cliente_id, $estado_id;
      $tot_curxinst_id = array() ;  
      $nm_comando = "select count(*), count(Cliente_id) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['where_pesq']; 
      ...
      $tot_curxinst_id[0] = NM_encode_input(sc_strip_script($curxinst_id)) ; 
      $tot_curxinst_id[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_curxinst_id[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_curxinst_id;
   } 
   ...
   {
      global $nm_lang, $cliente_id, $estado_id;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('curxinst_id' => "Curxinst_id");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('id_clixcur' => 'N','cliente_id' => 'N','curxinst_id' => 'N','estado_id' => 'N');
     $cmd_simp = "select count(*), count(Cliente_id)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
     $comando  = "select count(*), count(SC_metric1)#@#cmps_quebras#@# from (";
     $comando .= "select Cliente_id as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
     $comando .= "select " . $cmps_quebras2 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tb_clientesxcursos']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['curxinst_id']['reg'] = "S";
      $format_dimensions['curxinst_id']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "curxinst_id") {
                  $this->Lookup->lookup_sc_free_group_by_curxinst_id($$Cada_dim,  $curxinst_id);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
