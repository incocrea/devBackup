...
if (!isset($this->sc_temp_par_cliente_id)) {$this->sc_temp_par_cliente_id = (isset($_SESSION['par_cliente_id'])) ? $_SESSION['par_cliente_id'] : "";}
  ...
  if($this->sc_logged_is_blocked()) { if (isset($this->Campos_Mens_erro) && !empty($this->Campos_Mens_erro))
{
 ...
 if (isset($this->sc_temp_par_cliente_id)) { $_SESSION['par_cliente_id'] = $this->sc_temp_par_cliente_id;}
    $_SESSION['scriptcase']['app_login']['contr_erro'] = 'off';
    return;
}
}
...
; 
	...
	Cliente_email = $slogin"; 
      ...
      { 
		...
		$par_cliente_id = $this->rs_doc[0][0];
		 if (isset($par_cliente_id)) {$this->sc_temp_par_cliente_id = $par_cliente_id;}
;
	}		
...
;
	...
	if($this->sc_logged($this->login )):
		;
		$_SESSION['scriptcase']['user_logout'][] = array('V' => 'logged_user', 'U' => 'logout', 'R' => 'Login', 'T' => '_top');;
        if (is_file($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_vrcrenewable.txt")) {
    unlink($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_vrcrenewable.txt");
}
if (is_file("../_lib/friendly_url/Login_ini.txt")) {
    $SC_arq_def = fopen($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_vrcrenewable.txt", "w");
    fwrite ($SC_arq_def, 'Login, R');
    fclose ($SC_arq_def);
    setcookie('sc_apl_default_vrcrenewable','Login, R','0','/');
}
;
		 if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
