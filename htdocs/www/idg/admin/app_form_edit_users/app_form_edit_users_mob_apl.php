...
//
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users_mob']['dados_select']['mfa']) && !isset($this->nmgp_refresh_fields))
          { 
              $this->mfa = $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users_mob']['dados_select']['mfa'];
          } 
      }
...
//---------------------------------------------------------
    ...
    function ValidateField_mfa(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
        global $teste_validade;
        $hasError = false;
      if ($this->mfa == "" && $this->nmgp_opcao != "excluir")
      { 
      } 
        if ($hasError) {
            global $sc_seq_vert;
...
//
   ...
   function ajax_return_values_mfa($bForce = false)
   {
          if ('navigate_form' == $this->NM_ajax_opcao || 'backup_line' == $this->NM_ajax_opcao || (isset($this->nmgp_refresh_fields) && in_array("mfa", $this->nmgp_refresh_fields)) || $bForce)
          {
              ...
              $aLookup = array();
              $this->_tmp_lookup_mfa = $this->mfa;

$aLookup[] = array(app_form_edit_users_mob_pack_protect_string('Administrador') => str_replace('<', '&lt;',app_form_edit_users_mob_pack_protect_string("Administrador")));
$aLookup[] = array(app_form_edit_users_mob_pack_protect_string('Estudiante') => str_replace('<', '&lt;',app_form_edit_users_mob_pack_protect_string("Estudiante")));
$aLookup[] = array(app_form_edit_users_mob_pack_protect_string('Empresa') => str_replace('<', '&lt;',app_form_edit_users_mob_pack_protect_string("Empresa")));
$_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users_mob']['Lookup_mfa'][] = 'Administrador';
$_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users_mob']['Lookup_mfa'][] = 'Estudiante';
$_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users_mob']['Lookup_mfa'][] = 'Empresa';
          $aLookupOrig = $aLookup;
          $sSelComp = "name=\"mfa\"";
          if (isset($this->NM_ajax_info['select_html']['mfa']) && !empty($this->NM_ajax_info['select_html']['mfa']))
          {
              $sSelComp = str_replace('{SC_100PERC_CLASS_INPUT}', $this->classes_100perc_fields['input'], $this->NM_ajax_info['select_html']['mfa']);
          }
          $sLookup = '';
          if (empty($aLookup))
          {
          ...
          $aLabel     = array();
          $aLabelTemp = array();
          foreach ($this->NM_ajax_info['fldList']['mfa']['valList'] as $i => $v)
          {
              $this->NM_ajax_info['fldList']['mfa']['valList'][$i] = app_form_edit_users_mob_pack_protect_string($v);
          }
          foreach ($aLookupOrig as $aValData)
          {
              if (in_array(key($aValData), $this->NM_ajax_info['fldList']['mfa']['valList']))
              {
                  $aLabelTemp[key($aValData)] = current($aValData);
              }
          }
          foreach ($this->NM_ajax_info['fldList']['mfa']['valList'] as $iIndex => $sValue)
          {
              $aLabel[$iIndex] = (isset($aLabelTemp[$sValue])) ? $aLabelTemp[$sValue] : $sValue;
          }
          $this->NM_ajax_info['fldList']['mfa']['labList'] = $aLabel;
          }
   }
      ...
      if ($this->nmgp_opcao == "alterar") 
      {
          ...
          { 
              ...
              { 
                  ...
                  $SC_fields_update[] = "name = '$this->name', email = '$this->email', active = '$this->active', mfa = '$this->mfa'"; 
              } 
              else 
              { 
                  $comando = "UPDATE " . $this->Ini->nm_tabela . " SET ";  
             ...
             { 
              ...
              $this->nmgp_refresh_fields = array_diff(array('picture', 'login', 'pswd', 'confirm_pswd', 'name', 'email', 'mfa', 'active', 'groups'), $aDoNotUpdate);
              $this->ajax_return_values();
...
sc_send_mail_api(array(
    'profile' => '',
		...
		'settings' => [
				'gateway'       => 'smtp',
				'smtp_server'   => 'vrcrenewable.com',
				'smtp_port'     => '465',
				'smtp_user'     => 'contact@vrcrenewable.com',
				'smtp_password' => 'ViviRub_468*#',
				'from_email'    => 'contact@vrcrenewable.com',
				'from_name'    => 'contact@vrcrenewable.com',
		],
    'message' => [
...
function sc_file_size($file, $format = false)
{
 ...
 function new_date_format($type, $field)
 {
   ...
   function Form_lookup_mfa()
   {
       $nmgp_def_dados  = "";
       $nmgp_def_dados .= "Administrador?#?Administrador?#??@?";
       $nmgp_def_dados .= "Estudiante?#?Estudiante?#??@?";
       $nmgp_def_dados .= "Empresa?#?Empresa?#??@?";
