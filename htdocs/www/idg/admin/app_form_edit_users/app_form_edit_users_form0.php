...
function nav_desliga_img()
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "mfa")
     {
        $('select[name="mfa"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('select[name="mfa"]').addClass("scFormInputDisabled");
        }
        else {
            $('select[name="mfa"]').removeClass("scFormInputDisabled");
        }
     }
  }
 } // nm_field_disabled
...
<input type=hidden name="mfa" value="<?php echo $this->form_encode_input($this->mfa) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormLabelOdd scUiLabelWidthFix css_mfa_label" id="hidden_field_label_mfa" style="<?php echo $sStyleHidden_mfa; ?>"><span id="id_label_mfa"><?php echo $this->nm_new_label['mfa']; ?></span></TD>
    <TD class="scFormDataOdd css_mfa_line" id="hidden_field_data_mfa" style="<?php echo $sStyleHidden_mfa; ?>"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_mfa_line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["mfa"]) &&  $this->nmgp_cmp_readonly["mfa"] == "on") { 

$mfa_look = "";
 if ($this->mfa == "Administrador") { $mfa_look .= "Administrador" ;} 
 if ($this->mfa == "Estudiante") { $mfa_look .= "Estudiante" ;} 
 if ($this->mfa == "Empresa") { $mfa_look .= "Empresa" ;} 
 if (empty($mfa_look)) { $mfa_look = $this->mfa; }
?>
<input type="hidden" name="mfa" value="<?php echo $this->form_encode_input($mfa) . "\">" . $mfa_look . ""; ?>
<?php } else { ?>
<?php
...
$mfa_look = "";
 ...
 if (empty($mfa_look)) { $mfa_look = $this->mfa; }
?>
<span id="id_read_on_mfa" class="css_mfa_line"  style="<?php echo $sStyleReadLab_mfa; ?>"><?php echo $this->form_format_readonly("mfa", $this->form_encode_input($mfa_look)); ?></span><span id="id_read_off_mfa" class="css_read_off_mfa<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap; <?php echo $sStyleReadInp_mfa; ?>">
 <span id="idAjaxSelect_mfa" class="<?php echo $this->classes_100perc_fields['span_select'] ?>"><select class="sc-js-input scFormObjectOdd css_mfa_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_mfa" name="mfa" size="1" alt="{type: 'select', enterTab: false}">
 <option  value="Administrador" <?php  if ($this->mfa == "Administrador") { echo " selected" ;} ?>>Administrador</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users']['Lookup_mfa'][] = 'Administrador'; ?>
 <option  value="Estudiante" <?php  if ($this->mfa == "Estudiante") { echo " selected" ;} ?>>Estudiante</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users']['Lookup_mfa'][] = 'Estudiante'; ?>
 <option  value="Empresa" <?php  if ($this->mfa == "Empresa") { echo " selected" ;} ?>>Empresa</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_edit_users']['Lookup_mfa'][] = 'Empresa'; ?>
 </select></span>
</span><?php  }?>
