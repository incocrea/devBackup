<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="id_insumos_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_insumos_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tb_insumos_x_cursos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tb_insumos_x_cursos']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tb_insumos_x_cursos_submit_form(y, z);
...
function nm_mostra_doc(campo1, campo2, campo3)
{
    ...
    NovaJanela = window.open ("form_tb_insumos_x_cursos_doc.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nm_cod_doc=" + campo1 + "&nm_nome_doc=" + encodeURIComponent(campo2) + "&nm_cod_apl=" + campo3, "sc_name_download_iframe", "resizable, scrollbars");
}
