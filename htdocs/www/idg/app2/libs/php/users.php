...
$query = $this->db->query($str);			
	...
	function getPitems($info)
	{
		$OCODE = $info["OCODE"];
		
		
		$str = "SELECT TCODE, TNAME FROM turbines WHERE OCODE = '".$OCODE."' ORDER BY TNAME";
		$query = $this->db->query($str);
	...
	function getPjComents($info)
	{
		$PCODE = $info["PCODE"];
		
		
		$str = "SELECT COMMENTS FROM projects WHERE PCODE = '".$PCODE."'";
		$query = $this->db->query($str);
	...
	function savePjComents($info)
	{
		$COMMENTS = $info["COMMENTS"];
		$PCODE = $info["PCODE"];
		
		
		$str = "UPDATE projects SET COMMENTS = '".$COMMENTS."' WHERE PCODE ='".$PCODE."'";
		$query = $this->db->query($str);
	...
	function savePicsNew($info)
	{
		...
		$CODE = md5($ITEMCODE.$now);
		$TYPE1 = $info["TYPE1"];
		$TYPE2 = $info["TYPE2"];
		$FILE = $info["FILE"];
		$COMMENTS = $info["COMMENTS"];
		$LOADATE = $info["ITEMDATE"];
		
		$str = "INSERT INTO pics(
		...
		$str = "SELECT CODE, ITEMCODE, TYPE2, FILE, COMMENTS FROM pics WHERE ITEMCODE = '".$ITEMCODE."' AND LOADATE = '".$ITEMDATE."' ORDER BY TYPE2 ASC";
		$picList = $this->db->query($str);
	...
	function saveFilesNew($info)
	{
		...
		if($TYPE2 == "training")
		{
			...
			$str = "SELECT CODE, ITEMCODE, TYPE2, FILE, COMMENTS FROM pics WHERE ITEMCODE = '".$ITEMCODE."' ORDER BY TYPE2 ASC";
			$fileList = $this->db->query($str);
	...
	function getItemPics($info)
	{
		...
		if($PICKING == "ppeItemPic")
		{
			$ITEMCODE = $info["CODE"];
			$ITEMDATE = $info["EQDDATE"];
		}
		if($PICKING == "file")
		{
			$ITEMCODE = $info["CODE"];
		...
		{
			$str = "SELECT CODE, ITEMCODE, TYPE2, FILE, COMMENTS FROM pics WHERE ITEMCODE = '".$ITEMCODE."' AND TYPE1 = '".$ITEMDATE."' ORDER BY TYPE2 ASC";
		}
	...
	function pdfExport($info)
	{
		
		if($info["TYPE"] == "item" or $info["TYPE"] == "itemG")
		{
			...
			for($d=0;$d<count($ICODES);$d++)
			{
				
				$item = $ICODES[$d];
				
				// GET OCODE
				$str = "SELECT TNAME, OCODE FROM turbines WHERE TCODE = '".$item."'";
				$OCODE = $this->db->query($str)[0]["OCODE"];
				$TNAME = $this->db->query($str)[0]["TNAME"];
				
				// GET PCODE
				$str = "SELECT * FROM projects WHERE OCODE = '".$OCODE."'";
				$PDATA = $this->db->query($str)[0];
				
				
				// IF ITEM G THIS WILL ONLY BE ONCE

				if($d == 0)
				{
					// PAGE START HEADER
					$pdf->AddPage('P', 'Letter');
					$pdf->Image('../../irsc/reportBase.jpg',0,0,218,0,'','');
					...
					if($info["TYPE"] != "itemG")
					{
						$pdf->Cell(155,5,'PROJECT ITEMS FULL REPORT',0,0,'C',false);
					}
					else
					{
						$pdf->Cell(155,5,'PROJECT ITEM REPORT',0,0,'C',false);
					}
					
					
					
					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(20,5,"Project Number",0,0,'R',false);
					
					$pdf->Ln(5);
					
					$pdf->Cell(175,5,'',0,0,'C',false);
					$pdf->SetFont('Arial','B', 10);
					$pdf->Cell(20,5,$PDATA["QCODE"],0,0,'R',false);
					$pdf->SetFont('Arial','B', 8);
											
					$pdf->Ln(10);
					
					$pdf->Cell(20,5,"LOCATION:",0,0,'L',false);
					$pdf->Cell(80,5,$PDATA['WFARM'],0,0,'L',false);
					$pdf->Cell(20,5,"EQUIPMENT:",0,0,'L',false);
					$pdf->Cell(75,5,$PDATA['PLATFORM'],0,0,'L',false);
					
					
					$pdf->Ln(5);
					
					$pdf->Cell(20,5,"JOB TYPE:",0,0,'L',false);
					$pdf->Cell(40,5,$PDATA['JOB'],0,0,'L',false);
					$pdf->Cell(20,5,"JOB ORDER:",0,0,'L',false);
					$pdf->Cell(20,5,$PDATA['WO'],0,0,'L',false);
					$pdf->Cell(20,5,"CATEGORY:",0,0,'L',false);
					$pdf->Cell(40,5,$PDATA['G4'],0,0,'L',false);
					
					$pdf->Ln(5);
					
					$DETAIL = "DETAILS: ".$PDATA['DETAIL'];
					
					$w = array(195);
					$pdf->SetWidths($w);
					$align = array("L");
					$pdf->Row(array($DETAIL), 0, $align);
					
					$pdf->Ln(15);
					
					if($info["TYPE"] != "itemG")
					{
						$pdf->Cell(20,5,"ITEM:",0,0,'L',false);
						$pdf->Cell(175,5,$TNAME,0,0,'L',false);
					}
					...
					{
						$PCOMMENTS = json_decode($PDATA["COMMENTS"], true);
						$ini = $PCOMMENTS["INITIAL"];
						$pro = $PCOMMENTS["PROCESS"];
						$end = $PCOMMENTS["FINAL"];
					}
					else
					{
					...
					$iniComments = "OBSERVATIONS: ".$ini;
					$proComments = "PROCESS COMMENTS: ".$pro;
					$endComments = "CONCLUSIONS: ".$end;
					...
					$pdf->SetWidths($w);
					$align = array("L");
					$pdf->Row(array($iniComments), 1, $align);
					...
					$pdf->SetWidths($w);
					$align = array("L");
					$pdf->Row(array($proComments), 1, $align);
					...
					$pdf->SetWidths($w);
					$align = array("L");
					$pdf->Row(array($endComments), 1, $align);
				}
				...
				// GET ITEM PICS AND COMMENT PICS
				
				$str = "SELECT TYPE1, TYPE2, FILE, LOADATE, COMMENTS FROM pics WHERE ITEMCODE = '".$item."' ORDER BY LOADATE ASC, TYPE2 ASC";
				$pics = $this->db->query($str);
				...
				for($p=0;$p<count($pics);$p++)
				{
					...
					if($pics[$p]["TYPE2"] == "0"){$pType = "Initial Picture";}
					if($pics[$p]["TYPE2"] == "1"){$pType = "Process Picture";}
					if($pics[$p]["TYPE2"] == "2"){$pType = "Final Picture";}
					
					$pdate = explode(' ',$pics[$p]["LOADATE"])[0];
					
					$pDetail = $pType." "."Date: ".$pdate." ".$pics[$p]["COMMENTS"];
					...
					if($midCount == 0)
					{
						...
						$pdf->AddPage('P','Letter');
						$pdf->Ln(0);
						$pdf->Cell(50,5,"ITEM: ".$TNAME,0,0,'L',false);
						...
						$pdf->Cell(50,5,$pDetail,0,0,'L',false);
					...
					$imgPath = '../../irsc/tmpImg'.$p.'.jpg';
					
					if(file_put_contents($imgPath,$decodedImg) !== false)
					{
						$pdf->Image($imgPath,$picPosV,$picPosH,176,100);
						unlink($imgPath);
					}
				}
			}
			...
			$file = 'Item Report '.$PDATA["QCODE"].'.pdf';
			$path = '../../pdfReports/'.$file;
		...
		if($info["TYPE"] == "day" or $info["TYPE"] == "dayG")
		{
			...
			for($d=0;$d<count($DCODES);$d++)
			{
				...
				if($d == 0)
				{
					...
					// PAGE START HEADER
					$pdf->AddPage('L', 'A4');
					$pdf->Image('../../irsc/reportBaseH.jpg',0,0,290,0,'','');
					$pdf->SetFont('Arial','B', 8);
					...
					if($info["TYPE"] == "dayG")
					{
						$pdf->Cell(235,5,'PROJECT FULL REPORT - PROJECT COMPLETED: '.$progress."%",0,0,'C',false);
					}
					else
					{
						$pdf->Cell(235,5,'DAYLY REPORT - PROJECT COMPLETED: '.$progress."%",0,0,'C',false);
					}
					...
					$pdf->Cell(250,5,"",0,0,'R',false);
					$pdf->SetFont('Arial','B', 12);
					$pdf->Cell(20,5,$dData["QCODE"],0,0,'R',false);


					$pdf->Ln(10);
					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(135,5,"",0,0,'R',false);
					$pdf->Cell(20,5,"Project Totals:",0,0,'R',false);
					
					$pdf->Cell(20,5,"Turbines",1,0,'C',false);
					$pdf->Cell(20,5,"Mob In/Out",1,0,'C',false);
					$pdf->Cell(20,5,"Labor",1,0,'C',false);
					$pdf->Cell(20,5,"Standby",1,0,'C',false);
					$pdf->Cell(20,5,"NC",1,0,'C',false);
					$pdf->Cell(20,5,"Out of scope",1,0,'C',false);
					...
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(155,5,"",0,0,'R',false);
					$pdf->Cell(20,5,$total0,1,0,'C',false);
					$pdf->Cell(20,5,$total1,1,0,'C',false);
					$pdf->Cell(20,5,$total2,1,0,'C',false);
					$pdf->Cell(20,5,$total3,1,0,'C',false);
					$pdf->Cell(20,5,$total4,1,0,'C',false);
					$pdf->Cell(20,5,$total5,1,0,'C',false);
					...
					$pdf->Cell(20,5,"LOCATION: ",0,0,'L',false);
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(105,5,$qdata["WFARM"],0,0,'L',false);

					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(30,5,"SERVICE ORDER: ",0,0,'L',false);
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(10,5,$dData["SO"],0,0,'L',false);

					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(30,5,"WORK ORDER: ",0,0,'L',false);
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(10,5,$dData["WO"],0,0,'L',false);

					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(30,5,"CATEGORY: ",0,0,'L',false);
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(40,5,$qdata["G4"],0,0,'L',false);

					$pdf->Ln(5);
					
					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(20,5,"START DATE: ",0,0,'L',false);
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(20,5,$qdata["INIDATE"],0,0,'L',false);
					
					
					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(20,5,"JOB TYPE: ",0,0,'L',false);
					$pdf->SetFont('Arial','', 8);
					$pdf->Cell(20,5,$qdata["JOB"],0,0,'L',false);
					
					$pdf->Ln(5);
					
					$pdf->SetFont('Arial','B', 8);
					$pdf->Cell(20,5,"DESCRIPTION: ",0,0,'L',false);
					...
					$pdf->SetWidths($w);
					$align = array("L");
					$pdf->Row(array($qdata["DETAIL"]),0, $align);
					
					
					// DIVIDER
					$pdf->Ln(1);
					$pdf->Cell(275,0,"",1,0,'L',false);
				}
				else
				{
					// PAGE START HEADER
				...
				$pdf->Cell(20,5,"DATE ",1,0,'L',false);
				$pdf->Cell(40,5,"DAY STATUS ",1,0,'L',false);
				$pdf->Cell(155,5,"PLAN OF DAY ",1,0,'L',false);
				$pdf->Cell(30,5,"SERVICE ORDER ",1,0,'L',false);
				$pdf->Cell(30,5,"WORK ORDER ",1,0,'L',false);
				...
				$pdf->Cell(20,5,$dData["DATE"],1,0,'L',false);
				$pdf->Cell(40,5,$dData["WEATHER"],1,0,'L',false);
				$pdf->Cell(155,5,$dData["POD"],1,0,'L',false);
				$pdf->Cell(30,5,$dData["SO"],1,0,'L',false);
				$pdf->Cell(30,5,$dData["WO"],1,0,'L',false);
				...
				// LABORS TABLE AND COMMENTS
				
				
				$w = array(20, 32, 10, 14, 15, 25, 10, 25, 14, 25, 10, 25, 10, 10, 15, 15);
				$header = array('Responsible',
								'Name',
								'Quote',
								'Executed',
								'Mob In/out',
				...
				for($i=0;$i<count($header);$i++){$pdf->Cell($w[$i],5,utf8_decode($header[$i]),1,0,'L',true);}
				...
				$align = array("C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "C");
				...
				for($i = 0; $i<count($dItemsL);$i++)
				{
					...
					$responsible = utf8_decode($item["RESPONSIBLE"]);
					$name = utf8_decode($item["NAME"]);
					$quote = utf8_decode($item["QUOTE"]);
					
					if(isset($item["TE"])){$te = $item["TE"];}
					else{$te = "0";}
					...
					if($item["STDARRAY"] != "" and  $item["STDARRAY"] != null)
					{
						...
						for($s = 0; $s<count($sbItems);$s++)
						{
							...
							{
								$sbTotal = $sbTotal+$reasonQty;
								if($s == 0){$sbDetails .= $reasonDetail.": ".$reasonQty."h";}
								else{$sbDetails .= ", ".$reasonDetail.": ".$reasonQty."h";}
							}
						}
					}
					// GET SB ITEMS
					...
					$pdf->Row(array($responsible, $name, $quote,$te, $mimo, $mimoD, $labor, $laborD,$sbTotal, $sbDetails,$nc,$ncD,$ous, $ousD,$inihour,$endhour), 1, $align);

					$fill = !$fill;
					
					$t1 = $t1+floatval($mimo);
					$t2 = $t2+floatval($labor);
					$t3 = $t3+floatval($sbTotal);
					$t4 = $t4+floatval($nc);
					$t5 = $t5+floatval($ous);
				...
				$pdf->Row(array($bl,$bl,$bl,"Totals",$t1,$bl,$t2,$bl,$t3,$bl,$t4,$bl,$t5,$bl,$bl,$bl), 1, $align);
				$pdf->Cell(30,5,"LABOR GENERAL COMMENTS ",0,0,'L',false);
				$pdf->Ln(5);
				$pdf->SetFont('Arial','', 8);
				$w = array(275);
				$pdf->SetWidths($w);
				...
				// ITEMS TABLE AND COMMENTS
				$w = array(60, 10, 15, 20, 20, 20, 20, 98, 12);
				$header = array('Item',
								'Quote',
								'Executed',
								'Service Order',
				...
				for($i = 0; $i<count($dItemsT);$i++)
				{
					...
					$pdf->Row(array($name, $quote, $labor, $so, $wo, $iDate, $fDate, $comments, $priority), 1, $align);
					$fill = !$fill;
					
				}
				
				$pdf->SetFont('Arial','b', 8);
				$pdf->Cell(30,5,"ITEMS GENERAL COMMENTS ",0,0,'L',false);
				$pdf->Ln(5);
				$pdf->SetFont('Arial','', 8);
				$w = array(275);
				$pdf->SetWidths($w);
				$align = array("L");
				$pdf->Row(array($dData["TOBS"]), 1, $align);
				
				
				$pdf->Ln(5);
				
				
				// ADDED TABLE AND COMMENTS
				$w = array(210, 10, 15, 20, 20);
				$header = array('Added Cost',
								'Used',
								'Unit',
								'Quote',
								'Executed'
				...
				for($i = 0; $i<count($dItemsA);$i++)
				{
					...
					$pdf->Row(array($name, $labor, $responsible, $quote, $executed), 1, $align);
					$fill = !$fill;
				...
				$str = "SELECT TYPE1, TYPE2, FILE, LOADATE, COMMENTS FROM pics WHERE ITEMCODE = '".$dcode."' ORDER BY LOADATE ASC, TYPE2 ASC";
				$pics = $this->db->query($str);
				...
				for($p=0;$p<count($pics);$p++)
				{
					...
					if($midCount == 0)
					{
						...
						$pdf->AddPage('P','Letter');
						$pdf->Ln(0);
						$pdf->Cell(50,5,"DATE: ".$dData["DATE"],0,0,'L',false);
			...
			if($info["TYPE"] == "dayG")
			{
				$file = 'Full Report project '.$dData["QCODE"].'.pdf';
			}
			else
			{
				$file = 'Day Report '.$dData["QCODE"].'  '.$dData["DATE"].'.pdf';
			}
	...
	function newDeletePic($info)
	{
		$CODE = $info["CODE"];
		$ITEMCODE = $info["ITEMCODE"];
		
		$str = "DELETE FROM pics WHERE CODE = '".$CODE."'";
	...
	function getPodList($info)
	{
		...
		$str = "SELECT CODE, ONUM, CLIENT FROM orders WHERE STATUS != '1' ORDER BY ONUM DESC";
	...
	function podSave($info)
	{
		...
		if($otype == "c")
		{
			...
			$str = "INSERT INTO projects (PCODE, QCODE, WFARM, TURBINE, SO, WO, CO, PO, G4, INIDATE, JOB, DETAIL, PLATFORM, TOW,DEPARTMENT,PROPTYPE, WFTYPE, AREA, COUNTRY, TPLAT, TSERIE, OCODE) VALUES ('".$PCODE."', '".$QCODE."', '".$WFARM."', '".$TURBINE."', '".$SO."', '".$WO."', '".$CO."', '".$PO."', '".$G4."', '".$INIDATE."', '".$JOB."', '".$DETAIL."', '".$PLATFORM."' , '".$TOW."', '".$DEPARTMENT."', '".$PROPTYPE."', '".$WFTYPE."', '".$AREA."', '".$COUNTRY."', '".$TPLAT."', '".$TSERIE."', '".$OCODE."')";
			$query = $this->db->query($str);
	...
	function quoteApprove($info)
	{
		...
		$str = "UPDATE orders SET SERVORDER='".$serviceOrder."', STATUS = '2' WHERE CODE='".$code."'"; 
		$query = $this->db->query($str);
	...
	function tagContactOrder($info)
	{
		...
		if($etype == "day" or $etype == "dayG")
		{
			...
			$dpkc["mode"] = "";
			$dpkc["DCODE"] = $dcode;
			$createdPath = $this-> pdfExport($dpkc)["message"];
...
$query = $this->db->query($str);
	...
	function exportXLS($info)
	{
		...
		if($type == "order")
		{
			...
			$sheet->setCellValue("D$c", "Location");
			...
			$sheet->setCellValue("G$c", "Job Order");
			$sheet->setCellValue("J$c", "Quantity");
			...
			$sheet->getStyle("A$c:F$c")->getFont()->setSize(10);
			...
			$sheet->setCellValue("C$c", "Identified");
			$sheet->setCellValue("D$c", "Priority");
			$sheet->setCellValue("E$c", "Comments");
			$sheet->setCellValue("F$c", "Job order");
			
			// $sheet->setCellValue("C$c", "Wh# or Sf#");// $sheet->setCellValue("G$c", "Blade A S/N");
			...
			for($i = 0; $i<count($turbines);$i++)
			{
				...
				$sheet->setCellValue("C$c",  $item["IDDATE"]);
				$sheet->setCellValue("D$c",  $item["PRIORITY"]);
				$sheet->setCellValue("E$c",  $item["COMMENT"]);
				$sheet->setCellValue("F$c",  $item["WO"]);
				...
				$sheet->getStyle("A$c:F$c")->applyFromArray($alignLeft);
				$sheet->getStyle("A$c:F$c")->getFont()->setSize(9);
				// $sheet->getStyle("E$c")->getNumberFormat()->setFormatCode('#,##0.00');
				// $sheet->getStyle("I$c")->getNumberFormat()->setFormatCode('#,##0.00');
				// $sheet->getStyle("N$c")->getNumberFormat()->setFormatCode('#,##0.00');
				// $accessCost += $subCost;
		...
		if($type == "reqSup")
		{
			...
			$sheet->setCellValue("A$c", "Work/Support Request");
